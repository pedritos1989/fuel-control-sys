...
class DistribucionController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if ($reportForm->isSubmitted() && $reportForm->isValid()) {
            ...
            return $this->get('distribution.report.manager')->designReport($submitted);
        }

        $distribucions = $em->getRepository('CombBundle:Distribucion')->filter();
    ...
    public function createReportForm()
    {
        $form = $this->get('form.factory')
            ->createNamedBuilder('distribution_report')
            ->setAction($this->generateUrl('distribucion_index'))
            ->setMethod('GET');
        ...
        $form
            ...
            ->add('elaborado', TextType::class, array(
                'label' => 'distribution.report.write.name',
            ))
            ->add('cargoElab', TextType::class, array(
                'required' => false,
                'label' => 'distribution.report.charge',
            ))
            ->add('aprobado', TextType::class, array(
                'label' => 'distribution.report.aprove.name',
            ))
            ->add('cargoAprob', TextType::class, array(
                'required' => false,
                'label' => 'distribution.report.charge'
            ))
            ->add('revisado', TextType::class, array(
                'label' => 'distribution.report.review.name',
            ))
            ->add('cargoRev', TextType::class, array(
                'required' => false,
                'label' => 'distribution.report.charge'
            ))
